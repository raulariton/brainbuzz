...
export default async function getStatsBlocks(name, scope, id) {
    ...
    if (scope === 'all') {
        ...
        const lines = [
            `${name} team, you guys have:`,
            `- *Hosted a total of ${stats.total_quizzes} quiz(zes)*`,
            `- *Average score across all quizzes:* ${(stats.average_score * 100).toFixed(2)}%${stats.average_score > 70 ? ' (Great job! :trophy:)' : ''}`,
            `- *Average participation rate:* ${(stats.average_participation * 100).toFixed(2)}%${stats.average_participation > 50 ? ' (Nice work! :clap:)' : ''}`,
            `- *Most popular quiz type:* ${stats.most_popular_quiz_type}`,
            `- *The know-it-all:* <@${stats.highest_scoring_user.user_id}> with ${stats.highest_scoring_user.total_wins} wins and a win rate of ${(stats.highest_scoring_user.win_rate * 100).toFixed(2)}%`
        ];
